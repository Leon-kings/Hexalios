...
export const UserOrderManagement = () => {
  ...
  const fetchStatistics = async () => {
    ...
    try {
      ...
      // Filter stats by logged-in user's email
      const filteredStats = {
        totalOrders: allOrders.totalOrders || 0,
        totalRevenue: allOrders.totalRevenue || 0,
        paidCount: allOrders.paidCount || 0,
        paidAmount: allOrders.paidAmount || 0
      };
  ...
  // Handle search - only searches within logged-in user's orders
  const handleSearch = async () => {
    if (!loggedInUser?.email) return;
    
    if (!searchTerm.trim()) return fetchOrders();

    try {
      setLoading(true);
      const response = await axios.get(
        `https://hexaliosnode.onrender.com/orders/search?term=${searchTerm}&email=${loggedInUser.email}`
      );
      
      const results = response.data.data?.orders || [];
      setOrders(results);
      toast.info(results.length ? "Orders found" : "No orders found");
    } catch (err) {
  ...
  // Handle create - ensures new order has logged-in user's email
  const handleCreate = async () => {
    try {
      ...
      await axios.post("https://hexaliosnode.onrender.com/orders", orderToCreate);
  ...
  // Handle update - verifies order belongs to logged-in user
  const handleUpdate = async () => {
    try {
      if (currentOrder.customer.email !== loggedInUser.email) {
        throw new Error("Cannot update other users' orders");
      }
  ...
  const openEditModal = (order) => {
    ...
    if (order.customer.email === loggedInUser?.email) {
        ...
        <div className="p-4 md:p-6">
          ...
          <div className="mb-4 p-3 bg-blue-50 rounded-lg border border-blue-100">
            <p className="text-sm font-medium text-blue-800">
              Viewing orders for: <span className="font-bold">{loggedInUser?.email || "Not logged in"}</span>
            </p>
          </div>
          ...
          {stats && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              ...
              <div className="bg-white p-4 rounded-lg shadow">
                <h3 className="text-lg font-semibold text-gray-700">Your Revenue</h3>
                <p className="text-2xl font-bold">${stats.totalRevenue?.toFixed(2)}</p>
              </div>
              <div className="bg-white p-4 rounded-lg shadow">
                <h3 className="text-lg font-semibold text-gray-700">Paid Orders</h3>
                <p className="text-2xl font-bold">{stats.paidCount} (${stats.paidAmount?.toFixed(2)})</p>
              </div>
            </div>
          )}
