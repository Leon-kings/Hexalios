...
import { menProducts } from "../../../assets/data/data";
import axios from "axios";

export const MensLatest = () => {
  // State management
  ...
  useEffect(() => {
    const startAutoSlide = () => {
      autoSlideInterval.current = setInterval(() => {
        setCurrentSlide(
          (prev) =>
            (prev + 1) % Math.ceil(menProducts.length / productsPerPage.current)
        );
      }, 3000);
    };
  ...
  }, [menProducts.length, productsPerPage.current]);
  ...
  const goToPrevSlide = () => {
    setCurrentSlide(
      (prev) =>
        (prev - 1 + Math.ceil(menProducts.length / productsPerPage.current)) %
        Math.ceil(menProducts.length / productsPerPage.current)
    );
    pauseAutoSlide();
    resumeAutoSlide();
  };
                              ...
                              <div className="ml-4 flex-1">
                                ...
                                <div className="mt-1">
                                  ...
                                  <select
                                    ...
                                    className="ml-2 text-sm text-gray-500 border rounded p-1"
                                  >
                                    {item.sizes.map((size) => (
